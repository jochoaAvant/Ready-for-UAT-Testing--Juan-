...
def basic_validation(df1, df2, result_file, ignore_rows=0):
    ...
    if set(df1.columns) == set(df2.columns):
        ...
        for column in df1.columns:
            if df1[column].dtype != df2[column].dtype:
                ...
                result_file.write(
                    f"The column '{column}' has same data types in the dataframes.\n")
                print(
                    f"The column '{column}' has same data types in the dataframes.")
                new_row = pd.DataFrame([{'Column': column, 'Same DataType': 'yes',
                                       'DataTypeManual': df1[column].dtype, 'DataTypeAutomated': df2[column].dtype}])
                comparison_df = pd.concat([comparison_df,
                                          new_row], ignore_index=True)
...
def compare_df(df1, df2, result_file):
    ...
    if not diff.empty:
        ...
        result_file.write(
            f"The dataframes have the same columns and values.\n")
        return 0, "The dataframes have the same columns and values."


def main(vendor, filename, new_file, ignore_rows, error_codes):
    ...
    automated_url = Path(vendor, "test_files",
                         "rpm_files_automation", filenamea)
    output_url = Path(vendor, "test_files", "output",
    ...
    error, output_df = basic_validation(
        manual, automated, test_results, ignore_rows=ignore_rows)
    ...
    match error:
        case 1 | 2 | 4 | 5:
            ...
            test_results.write(error_codes[error])
        ...
        case 3 | 6:
            ...
            error, df_differences = compare_df(
...
if __name__ == "__main__":
    ...
    main(vendor=vendor, filename=filename, new_file=new_file, ignore_rows=ignore_rows,
